...
def create_database(path_cm, path_score, postfix, path_output):
    ...
    for f in score_list:
        df_ = pd.read_csv(f)
        df_score = pd.concat([df_score, df_])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--score', type=str, required=True,
                        help='The path for scores')
    parser.add_argument('-p', '--postfix', type=str, required=True,
                        help='The postfix of score name')
    ...
    create_database(path_cm, path_score, postfix, path_output)
